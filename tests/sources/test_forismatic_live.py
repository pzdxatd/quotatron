import pytest
from quotatron.sources.forismatic import ForismaticSource


@pytest.mark.live
@pytest.mark.asyncio
async def test_forismatic_returns_at_least_one_item() -> None:
    items = await ForismaticSource().fetch(limit=3)
